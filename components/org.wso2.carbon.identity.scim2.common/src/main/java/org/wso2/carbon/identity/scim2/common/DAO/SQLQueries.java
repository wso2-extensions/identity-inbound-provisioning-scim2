...
public class SQLQueries {
    ...
    public static final String LIST_SCIM_GROUPS_WITH_PAGINATION_SQL = "SELECT ROLE_NAME FROM IDN_SCIM_GROUP " +
            "WHERE IDN_SCIM_GROUP.ATTR_NAME = ? ORDER BY ROLE_NAME ASC LIMIT ? OFFSET ?";
    public static final String LIST_SCIM_GROUPS_WITH_PAGINATED_DB2 =
            "SELECT ROLE_NAME FROM(SELECT ROW_NUMBER() OVER (ORDER BY" +
                    " ROLE_NAME) AS rn, U.*  FROM IDN_SCIM_GROUP AS U) " +
                    "WHERE IDN_SCIM_GROUP.ATTR_NAME = ? AND rn BETWEEN ? AND ?";
    public static final String LIST_SCIM_GROUPS_WITH_PAGINATED_MSSQL = "SELECT ROLE_NAME FROM (SELECT ROLE_NAME," +
            "UM_TENANT_ID, ROW_NUMBER() OVER (ORDER BY ROLE_NAME) AS RowNum FROM IDN_SCIM_GROUP) AS P WHERE P" +
            ".IDN_SCIM_GROUP.ATTR_NAME = ? AND P.RowNum BETWEEN ? AND ?";
    public static final String LIST_SCIM_GROUPS_WITH_PAGINATED_ORACLE = "SELECT ROLE_NAME FROM (SELECT ROLE_NAME," +
            " UM_TENANT_ID, rownum AS rnum FROM (SELECT ROLE_NAME, UM_TENANT_ID FROM IDN_SCIM_GROUP ORDER BY " +
            "ROLE_NAME) WHERE IDN_SCIM_GROUP.ATTR_NAME = ? AND UM_TENANT_ID=? AND rownum <= ?) WHERE  rnum > ?";
