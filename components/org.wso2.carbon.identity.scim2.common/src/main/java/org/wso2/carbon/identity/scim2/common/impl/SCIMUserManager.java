...
public class SCIMUserManager implements UserManager {
    ...
    public User createUser(User user, Map<String, Boolean> requiredAttributes)
        ...
        try {
            ...
            org.wso2.carbon.user.core.common.User coreUser;
    ...
    private long getTotalUsers(String domainName) throws CharonException {
        ...
        try {
            ...
            String errorMsg = "Error while getting total user count in domain: " + domainName;
    ...
    public User updateUser(User user, Map<String, Boolean> requiredAttributes) throws CharonException,
        ...
        try {
            ...
            if (isLoginIdentifiresEnabled() && StringUtils.isNotEmpty(getPrimaryLoginIdentifireClaim())) {
                claimValuesInLocalDialect.put(getPrimaryLoginIdentifireClaim(), user.getUsername());
            }
    ...
    private User getSCIMUser(org.wso2.carbon.user.core.common.User coreUser, List<String> claimURIList,
        ...
        try {
            ...
            if (isLoginIdentifiresEnabled() && StringUtils.isNotEmpty(getPrimaryLoginIdentifireClaim())) {
                String primaryLoginIdentifier = userClaimValues.get(getPrimaryLoginIdentifireClaim());
            ...
            if (isSCIMEnabled(userStoreDomainName)) {
                ...
                try {
                    ...
                    if (mandateDomainForUsernamesAndGroupNamesInResponse()) {
                        if (isLoginIdentifiresEnabled() && StringUtils.isNotEmpty(getPrimaryLoginIdentifireClaim())) {
                            String primaryLoginIdentifier = userClaimValues.get(getPrimaryLoginIdentifireClaim());
                            if(primaryLoginIdentifier != null){

                                attributes.put(SCIMConstants.UserSchemaConstants.USER_NAME_URI, prependDomain(primaryLoginIdentifier));
                            }else{
    ...
    private Set<org.wso2.carbon.user.core.common.User> getUserNames(String attributeName, String filterOperation,
        ...
        if (SCIMConstants.UserSchemaConstants.USER_NAME_URI.equals(attributeName) && isLoginIdentifiresEnabled() &&
                StringUtils.isNotEmpty(getPrimaryLoginIdentifireClaim())) {
            attributeNameInLocalDialect = getPrimaryLoginIdentifireClaim();
        } else {
     ...
     */
    private boolean isLoginIdentifiresEnabled() {

        String enableLoginIdentifires = IdentityUtil
                .getProperty(SCIMCommonConstants.ENABLE_LOGIN_IDENTIFIRES);
        if (StringUtils.isEmpty(enableLoginIdentifires)) {

            // Return false if the user has not enabled the detailed response body.
            return SCIMCommonConstants.DEFAULT_ENABLE_LOGIN_IDENTIFIRES;
        } else {
            return Boolean.parseBoolean(enableLoginIdentifires);
        }
    }
     ...
     * @return
     */
    private String getPrimaryLoginIdentifireClaim() {

        return IdentityUtil.getProperty(SCIMCommonConstants.PRIMARY_LOGIN_IDENTIFIRE_CLAIM);
