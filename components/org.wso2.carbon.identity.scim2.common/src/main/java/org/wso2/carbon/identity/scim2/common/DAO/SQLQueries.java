...
public class SQLQueries {
    ...
    public static final String LIST_SCIM_GROUPS_WITH_FILTER_PAGINATED_MSSQL =
            "SELECT DISTINCT ROLE_NAME FROM (SELECT DISTINCT ROLE_NAME," +
                    "TENANT_ID, ROW_NUMBER() OVER (ORDER BY ROLE_NAME) AS RowNum FROM IDN_SCIM_GROUP) AS P WHERE P" +
                    ".ROLE_NAME LIKE ? AND P.TENANT_ID= ? AND P.RowNum BETWEEN ? AND ?";
    ...
    public static final String LIST_SCIM_GROUPS_WITH_PAGINATED_MSSQL = "SELECT ROLE_NAME FROM (SELECT ROLE_NAME," +
            "TENANT_ID, ROW_NUMBER() OVER (ORDER BY ROLE_NAME) AS RowNum FROM IDN_SCIM_GROUP) AS P WHERE P" +
            ".IDN_SCIM_GROUP.ATTR_NAME = ? AND TENANT_ID=? AND P.RowNum BETWEEN ? AND ?";
